...
import abm.util.Matrix;
import static abm.env.comp.GraphSites.*;
import static abm.util.Graph.*;

/**
 * Abstract class to hold utility functions used by GraphSites.
 *
...
abstract class GraphSitesUtilities {
	...
	private static final double OXY_PRESSURE_MAX = 100; // mmHg
	private static final double OXY_PRESSURE_MIN = 55; // mmHg
	private static final double OXY_PRESSURE_SCALE = 1; // mmHg/um
	private static final double OXY_CURVE_EXP = 2.8275;
	private static final double OXY_CURVE_P50 = 26.875; // mmHg
	private static final double OXYGEN_SATURATION = 0.00835; // fmol/um^3 blood
	private static final double CAP_EXP = 2.7;
	private static final double MIN_FLOW = 1000; // um^3/min
	...
	// METHOD: calcPressure. Calculates nodal pressure given radius (REF Welter, 2016).
	static double calcPressure(double radius, int type) {
		return 18 + (89 - 18)/(1 + Math.exp((radius*type + 21)/16));
	}
	
	// METHOD: calcViscosity. Calculates relative viscosity given radius (REF Pries, 1994).
	private static double calcViscosity(double r) {
		double D = 2*r;
	...
	// METHOD: getCoeff. Returns coefficient for flow rate in um^3/(mmHg min).
	...
	// METHOD: getPartial. Returns partial pressure for root edge radius.
	static double getPartial(SiteEdge edge) {
		return Math.min(OXY_PRESSURE_MIN + OXY_PRESSURE_SCALE*edge.radius, OXY_PRESSURE_MAX);
	}
	
	// METHOD: getSaturation. Returns oxygen saturation given partial pressure.
	...
	// METHOD: getTotal. Returns total oxygen concentration in blood in fmol/um^3.
	...
	// METHOD: getRootPressure. Gets max (for arteries) or min (for veins) pressure
	// across roots.
	...
	// METHOD: setRootPressures. Sets pressures of roots. Assumes that the root
	// node has already been set to the correct node object.
	...
	// METHOD: setLeafPressures. Sets pressure of leaves.
	...
	// METHOD: reversePressures. Reverse edges that have negative pressure difference.
	...
	// METHOD: checkPerfused. Marks perfused edges between given arteries and veins.
	...
	// METHOD: mergeGraphs. Merges the node from graph 2 into graph 1.
	...
	// METHOD: calcPressures. Calculates nodal pressure given set of non-root nodes.
	...
	// METHOD: calcStress. Calculates shear and circumferential stress along all edges.
	...
	// METHOD: calcFlows. Calculates flow rate across all edges in um^3/min and
	// area in um^2.
	...
	// METHOD: calcThicknesses. Calculates wall thickness in um.
	...
	// METHOD: calcRadius. Calculates radius using Murray's law.
	private static ArrayList<SiteEdge> calcRadius(Graph G, SiteEdge edge,
			int code, int dir, int fromcheck, int tocheck) {
		...
		for (Edge obj : list) {
			...
			int in = G.getInDegree(e.getNode(dir));
			int out = G.getOutDegree(e.getNode(dir));
			
			if (in == 1 && out == 1 && edge.radius != 0) { e.radius = edge.radius; }
			else if (in == fromcheck && out == tocheck) {
				ArrayList<Edge> b = (code == DOWNSTREAM ? edge.getEdgesOut() : edge.getEdgesIn());
				...
				ArrayList<Edge> b = (code == DOWNSTREAM ? e.getEdgesIn() : e.getEdgesOut());
				double r1 = ((SiteEdge)b.get(0)).radius;
				double r2 = ((SiteEdge)b.get(1)).radius;
	...
	// METHOD: assignRadius. Calculates radius using Murray's law without splits.
	...
	// METHOD: visit. Traverse through graph and mark visited nodes.
	...
	// METHOD: path. Uses Dijkstra's algorithm to find path between given nodes.
	...
	// METHOD: traverse. Traverse through graph and find path between given nodes
	...
	// METHOD: updateRadii. Calculates radii using Murray's law.
	...
	// METHOD: updateGraph. Updates graph hemodynamic properties when edges are
	// removed (and therefore marked as ignored).
	...
	// METHOD: updateTraverse. Iterates through nodes to eliminate low flow edges
	// that are preventing graph traversal.
	...
	// METHOD: getEdgeByType. Returns list of edges of the given type(s).
	...
	// METHOD: getEdgeByType. Returns list of edges of the given type(s) at given level.
	...
	// METHOD: getLeavesByType. Returns list of edges of the given type(s).
