...
public abstract class GraphSites extends Sites {
     ...
     *
     * @param component
     *            the parsed component attributes
     */
    GraphSites(MiniBox component) {
        ...
        for (String spec : specList) {
            specs.put(spec, component.get(spec));
        }
    }
     ...
     *
     * @param from
     *            the node the edge extends from
     * @param to
     *            the node the edge extends to
     ...
     *
     * @param node0
     *            the node the motif starts at
     * @param dir
     *            the direction code of the root
     * @param type
     *            the root type
     * @param bag
     *            the bag of active edges
     * @param scale
     *            the graph resolution scaling
     * @param level
     *            the graph resolution level
     * @param offsets
     *            the list of offsets for line roots, null otherwise
     */
     ...
     *            the node the motif starts at
     * @param dir
     *            the direction code for the edge
     * @param type
     *            the edge type
     * @param bag
     *            the bag of active edges
     ...
     *            the graph resolution level
     * @param e
     *            the edge the motif is being added to
     * @param motif
     *            the motif type
     */
     ...
     * @param dir
     *            the direction code for the segment
     * @param scale
     *            the graph resolution scaling
     * @param level
     *            the graph resolution level
     ...
     *
     * @param border
     *            the border the root extends from
     * @param perc
     *            the percentage distance across the border the root is located
     * @param type
     *            the root type
     * @param frac
     *            the fraction distance in the perpendicular direction the root
     *            extends
     * @param scale
     *            the graph resolution scaling
     ...
     * {@link arcade.env.comp.SourceSites}
     ...
     * @param x
     *            the coordinate in the x direction
     * @param y
     *            the coordinate in the y direction
     * @param z
     *            the coordinate in the z direction
     */
    ...
    /**
     * Extension of {@link arcade.env.comp.Site} for
     * {@link arcade.env.comp.GraphSites}.
     * <p>
    ...
    private class GraphSite extends Site {
         ...
         *            the molecule code
         * @param delta
         *            the array holding change in concentration
         * @param current
         *            the array holding current concentrations for current tick
         * @param previous
         *            the array holding previous concentrations for previous tick
         * @param conc
         *            the source concentration of the molecule
         * @param perm
         *            the permeability of the molecule
         */
    ...
    static class Root {
        ...
        /**
         * Creates a {@code Root} object for {@link arcade.env.comp.GraphSites} with a
         * root layout.
         *
         * @param x
         *            the x coordinate
    ...
    private ArrayList<SiteNode> traverseNode(SiteNode node, int code) {
        ...
        if (out != null) {
            ...
            // Check for same sign.
            if (Math.signum(func.f(MAX_OXYGEN_PARTIAL_PRESSURE)) == -1 || FLOW_OUT == 0) {
            ...
            for (SiteEdge edge1 : set) {
                if (G.getOutDegree(edge1.getTo()) == 1) {
                    ...
                    if (dir1 == dir2 && edge1.type == edge2.type) {
                        SiteEdge join = new SiteEdge(edge1.getFrom(), edge2.getTo(), edge1.type,
                                edge1.level + edge2.level);
     ...
     *
     * @param arteries
     *            the list of artery edges
     * @param veins
     *            the list of vein edges
     * @param scale
     *            the graph resolution scaling
     * @param level
     *            the graph resolution level
    ...
    private void updateGrowthSites(ArrayList<Root> arteries, ArrayList<Root> veins, int scale, int level) {
        ...
        switch (level) {
            ...
            case LEVEL_2:
                for (Root artery : arteries) {
                    artery.edge = (SiteEdge) G.getEdgesOut(artery.node).get(0);
                }
                for (Root vein : veins) {
                    vein.edge = (SiteEdge) G.getEdgesOut(vein.node).get(0);
                }
                break;
        }
     ...
     *
     * @param node
     *            the node of the initial location
     * @param offset
     ...
     *
     * @param bag
     *            the current bag of active edges
     * @param scale
     *            the graph resolution scaling
     * @param level
     *            the graph resolution level
    ...
    private double remodelSites(int scale, int level) {
        ...
        for (Object obj : allEdges) {
            ...
            if (rand < wD) {
                if (G.getOutDegree(to) == 0 && G.getInDegree(to) == 0) {
                    edge.tag = TO_REMOVE;
                    count++;
                }
            } else if (rand < wG) {
    ...
    public String toString() {
        ...
        String sites = "";
        for (int i = 0; i < 4; i++) {
            if (!"".equals(siteSetup[i])) {
                sites += " [" + labels[i] + " = " + siteSetup[i] + "]";
            }
        }
