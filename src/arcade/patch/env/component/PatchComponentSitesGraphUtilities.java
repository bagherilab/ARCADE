...
abstract class PatchComponentSitesGraphUtilities {
    ...
    static EdgeType parseType(String type) {
        return (type.equalsIgnoreCase("A") ? EdgeType.ARTERY
     ...
     * <p>
     * Equation based on the paper: Welter M, Fredrich T, Rinneberg H, and
     * Rieger H. (2016). Computational model for tumor oxygenation applied to
     * clinical data on breast tumor hemoglobin concentrations suggests vascular
     ...
     * <p>
     * Equation based on the paper: Pries AR, Secomb TW, Gessner T, Sperandio
     * MB, Gross JF, and Gaehtgens P. (1994). Resistance to blood flow in
    ...
    private static double calculateViscosity(double radius) {
        ...
        double mu45 = 6 * Math.exp(-0.085 * diameter) + 3.2 - 2.44
                * Math.exp(-0.06 * Math.pow(diameter, 0.645));
    ...
    private static double getSaturation(double pressure) {
        return Math.pow(pressure, OXYGEN_CURVE_EXP) / (Math.pow(pressure, OXYGEN_CURVE_EXP)
    ...
    /**
     * Calculates flow rate (in um<sup>3</sup>/min) and area (in um<sup>2</sup>)
     * for all edges.
     *
     * @param graph  the graph object
     */
    ...
    private static ArrayList<SiteEdge> calculateRadius(Graph graph, SiteEdge edge,
        ...
        for (Edge obj : list) {
            ...
            if (in == 1 && out == 1 && edge.radius != 0) {
                ...
                ArrayList<Edge> b = (dir == CalculationCategory.DOWNSTREAM
                        ? edge.getEdgesOut() : edge.getEdgesIn());
                ...
                if (e.radius == 0 && edge.radius != 0) {
                    if (r1 == 0 && r2 != 0) {
                        if (edge.radius > r2) {
                            e.radius = Math.pow(Math.pow(edge.radius, MURRAY_EXPONENT)
                                    - Math.pow(r2, MURRAY_EXPONENT), 1 / MURRAY_EXPONENT);
                            ...
                            e.radius = Math.pow(Math.pow(r2, MURRAY_EXPONENT)
                                    - Math.pow(edge.radius, MURRAY_EXPONENT), 1 / MURRAY_EXPONENT);
                        ...
                        if (edge.radius > r1) {
                            e.radius = Math.pow(Math.pow(edge.radius, MURRAY_EXPONENT)
                                    - Math.pow(r1, MURRAY_EXPONENT), 1 / MURRAY_EXPONENT);
     ...
     */
    static void visit(Graph graph, PatchComponentSitesGraphFactory gs, SiteNode node,
