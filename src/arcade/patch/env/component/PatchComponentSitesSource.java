 ...
 *
 * <p>Each index in the lattice can be assigned as a source, depending on the initial spacings in
 * the x direction (length, {@code X_SPACING}), y direction (width, {@code Y_SPACING}), and z
 * direction (height, {@code Z_SPACING}). Each spacing can be defined as:
 *
 * <ul>
 *   <li>{@code *} = all indices in the lattice
 *   <li>{@code x} or {@code X} = no indices in the lattice
 *   <li>{@code *:INC} = all indices in the lattice incremented by INC (i = 0, INC, 2*INC, ... ,
 *       n*INC) where n*INC &le; lattice size
 *   <li>{@code INDEX} = only index INDEX
 *   <li>{@code MIN:MAX} = all indices between MIN and MAX, inclusive
 *   <li>{@code MIN:INC:MAX} = all indices between MIN and MAX incremented by INC (i = MIN, MIN +
 *       INC, MIN + 2*INC, ... , MIN + n*INC) where MIN + n*INC &le; MAX
 * </ul>
 *
 * <p>Setting all spacings to {@code *} creates a constant source. Setting any spacings to {@code x}
 * removes all sites.
 *
 * <p>The amount of concentration added to each index is the difference between the concentration at
 * the index and the source concentration of the molecule. Sites can be damaged by setting the
 * {@code DAMAGE_SCALING} parameter, which reduces the amount of concentration added at each index.
 */
public class PatchComponentSitesSource extends PatchComponentSites {
     ...
     *
     * <ul>
     *   <li>{@code X_SPACING} = spacing of sources in x direction
     *   <li>{@code Y_SPACING} = spacing of sources in y direction
     *   <li>{@code Z_SPACING} = spacing of sources in z direction
     *   <li>{@code DAMAGE_SCALING} = source site damage scaling
     * </ul>
     *
     ...
     *
     * <p>Iterates through each index in the source lattice and assigns it as a source site or not,
     * depending on the specified spacings.
     */
            ...
            for (int k = 0; k < latticeHeight; k++) {
                for (int i = 0; i < latticeLength; i++) {
                    for (int j = 0; j < latticeWidth; j++) {
                        if (sources[k][i][j]) {
                            damageValues[k][i][j] =
                                    1.0 / Math.exp(damageScaling * damageSingle[k][i][j]);
                        }
                    }
                }
            }
