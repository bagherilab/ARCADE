 ...
 *
 * <p>This component can only be used with {@link PatchComponentSitesSource}. Multiple cycled
...
public class PatchComponentCycle implements Component {
    ...
    public void register(Simulation sim, String layer) {
        ...
        if (siteLayer != null) {
            CycleLayer cycleLayer =
                    new CycleLayer(layer, siteLayer, (PatchOperationGenerator) generator);
    ...
    public void step(SimState simstate) {
        ...
        double scale =
                (Math.tanh((x - Math.floor(x) - 0.5) * SMOOTHING_FACTOR))
                                / (2 * Math.tanh(0.5 * SMOOTHING_FACTOR))
                        + Math.floor(x)
                        + 1.5
                        - x;
