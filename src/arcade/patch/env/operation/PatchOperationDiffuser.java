 ...
 *
 * <p>Operation calculates diffusion of concentrations using finite difference approximation with
 * given {@code DIFFUSIVITY}. The calculation is repeated per second (model tick is one minute).
 * Methods are written to work regardless of underlying geometry. Methods extending this operation
 * for a specific geometry will need to adjust the multipliers for both the finite difference
...
public abstract class PatchOperationDiffuser extends PatchOperation {
     ...
     *
     * <p>Six border arrays are used to check if an index is located at the right/left ({@code
     * LENGTH}, x axis), top/bottom ({@code WIDTH}, y axis), and up/down ({@code HEIGHT}, z axis)
     * directions.
     *
     * <p>Loaded parameters include:
     *
     * <ul>
     *   <li>{@code DIFFUSIVITY} = diffusivity of molecule
     * </ul>
     *
