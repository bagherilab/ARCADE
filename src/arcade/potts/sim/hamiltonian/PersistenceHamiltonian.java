...
public class PersistenceHamiltonian implements Hamiltonian {
    ...
    public void register(PottsCell cell) {
        ...
        PersistenceHamiltonianConfig config =
                new PersistenceHamiltonianConfig(loc, lambda, lambdasRegion, decay, threshold);
     ...
     * {@inheritDoc}
     *
     * <p>Persistence energy is calculated by taking the sum of source and target persistence
     * energies. Source energy is calculated from displacement when a voxel is removed. Target
     * energy is calculated from displacement when a voxel is added.
     */
    @Override
     ...
     *
     * <p>Persistence energy is calculated by taking the sum of source and target persistence
     * energies for the region. Source energy is calculated from displacement when a voxel is
     * removed. Target energy is calculated from displacement when a voxel is added.
     */
    @Override
     ...
     *
     * <p>Persistence for non-cell voxels is zero.
     *
     ...
     *
     * <p>Persistence for the default region is zero. Calculating persistence for a region does not
     * update the target vector or displacement vectors for the cell.
     *
     * @param id the voxel id
                ...
                for (Region region : regionKeys) {
                    double lambdaRegion =
                            parameters.getDouble(
                                    "persistence/LAMBDA_" + region.name() + TARGET_SEPARATOR + key);
