...
public class HeightHamiltonian implements Hamiltonian {
     ...
     * {@inheritDoc}
     *
     * <p>Height energy is calculated by taking the difference in target and proposed height for the
     * given ID. Change in height energy is taken as the difference in differences of height
     ...
     *
     * <p>Height energy is calculated by taking the difference in target and proposed height for the
     * given region. Change in height energy is taken as the difference in differences of height
    ...
    public double getDelta(int id, int sourceRegion, int targetRegion, int x, int y, int z) {
        Voxel voxel = new Voxel(x, y, z);
        double source =
                getHeight(id, voxel, sourceRegion, -1) - getHeight(id, voxel, sourceRegion, 0);
        double target =
                getHeight(id, voxel, targetRegion, 1) - getHeight(id, voxel, targetRegion, 0);
        return target + source;
    }
    ...
    double getHeight(int id, Voxel voxel, int change) {
        ...
        ArrayList<Voxel> voxels =
                (ArrayList<Voxel>)
                        config.location.getVoxels().stream()
    ...
    double getHeight(int id, Voxel voxel, int t, double change) {
        ...
        ArrayList<Voxel> voxels =
                (ArrayList<Voxel>)
                        config.location.getVoxels(region).stream()
                                .filter(v -> v.x == voxel.x && v.y == voxel.y)
                                .collect(Collectors.toList());
                ...
                for (Region region : regionKeys) {
                    double lambdaRegion =
                            parameters.getDouble(
                                    "height/LAMBDA_" + region.name() + TARGET_SEPARATOR + key);
