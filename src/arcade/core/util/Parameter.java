 ...
 *
 * <p>Each {@code Parameter} is associated with a random number generator to select additional
 * parameter values from a normal distribution. For parameters that are fractions (bounded between 0
 * and 1), the tails of the distribution are truncated.
 */
public class Parameter implements Serializable {
    ...
    public Parameter(double mu, double h, boolean isFrac, MersenneTwisterFast random) {
        ...
        if (isFrac) {
            if (mu <= 0 || mu >= 1) {
                throw new OOBException("Mu parameter must be between 0 and 1, given " + mu + ".");
            }
