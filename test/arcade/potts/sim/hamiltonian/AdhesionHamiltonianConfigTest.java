...
public class AdhesionHamiltonianConfigTest {
    ...
    public void getAdhesion_validRegions_returnsValue() {
        ...
        AdhesionHamiltonianConfig ahc =
                new AdhesionHamiltonianConfig(null, new double[] {}, adhesionRegion);

        for (Region target : regions) {
            assertEquals(
                    defaultAdhesion.get(target), ahc.getAdhesion(Region.DEFAULT, target), EPSILON);
            assertEquals(
                    nucleusAdhesion.get(target), ahc.getAdhesion(Region.NUCLEUS, target), EPSILON);
        }
    }
