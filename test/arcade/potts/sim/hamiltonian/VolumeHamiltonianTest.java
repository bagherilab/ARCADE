...
public class VolumeHamiltonianTest {
    ...
    public void constructor_called_initializesParameters() {
        ...
        series.potts.put(
                "volume/LAMBDA_" + Region.DEFAULT.name() + TARGET_SEPARATOR + key2, lambdaDefault);
        series.potts.put(
                "volume/LAMBDA_" + Region.NUCLEUS.name() + TARGET_SEPARATOR + key2, lambdaNucleus);
    ...
    public void getDelta_validIDs_calculatesValue() {
        ...
        assertEquals(
                ...
                vh.getDelta(id1, id2, 0, 0, 0),
                EPSILON);
        assertEquals(
                (cell2minus1 - cell2 + cell1plus1 - cell1),
                vh.getDelta(id2, id1, 0, 0, 0),
                EPSILON);
    }
    ...
    public void getDelta_validRegions_calculatesValue() {
        ...
        assertEquals(
                (regionplus1 - region),
                vh.getDelta(id, Region.DEFAULT.ordinal(), Region.NUCLEUS.ordinal(), 0, 0, 0),
                EPSILON);
        assertEquals(
                (regionminus1 - region),
                vh.getDelta(id, Region.NUCLEUS.ordinal(), Region.DEFAULT.ordinal(), 0, 0, 0),
                EPSILON);
    }
    ...
    public void getVolume_validID_calculatesValue() {
        ...
        assertEquals(
                lambda * Math.pow(volume - targetVolume - 1, 2), vh.getVolume(id, -1), EPSILON);
    }
    ...
    public void getVolume_validRegions_calculatesValue() {
        ...
        assertEquals(
                lambda * Math.pow(volume - targetVolume, 2),
                vh.getVolume(id, region.ordinal(), 0),
                EPSILON);
        assertEquals(
                lambda * Math.pow(volume - targetVolume + 1, 2),
                vh.getVolume(id, region.ordinal(), 1),
                EPSILON);
        assertEquals(
                lambda * Math.pow(volume - targetVolume - 1, 2),
                vh.getVolume(id, region.ordinal(), -1),
