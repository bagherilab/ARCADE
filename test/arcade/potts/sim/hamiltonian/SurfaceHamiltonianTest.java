...
public class SurfaceHamiltonianTest {
    ...
    static class SurfaceHamiltonianMock extends SurfaceHamiltonian {
        SurfaceHamiltonianMock(PottsSeries series, Potts potts) {
            super(series, potts);
        }
    ...
    public void constructor_called_initializesMaps() {
        SurfaceHamiltonianMock shm =
                new SurfaceHamiltonianMock(mock(PottsSeries.class), mock(Potts.class));
    ...
    public void constructor_called_initializesParameters() {
        ...
        series.potts.put(
                "surface/LAMBDA_" + Region.DEFAULT.name() + TARGET_SEPARATOR + key2, lambdaDefault);
        series.potts.put(
                "surface/LAMBDA_" + Region.NUCLEUS.name() + TARGET_SEPARATOR + key2, lambdaNucleus);
    ...
    public void getDelta_validIDs_calculatesValue() {
        SurfaceHamiltonianMock shm =
                spy(new SurfaceHamiltonianMock(mock(PottsSeries.class), mock(Potts.class)));
        ...
        assertEquals(
                (cell1plus1 - cell1 + cell2minus1 - cell2),
                shm.getDelta(id1, id2, 0, 0, 0),
                EPSILON);
        assertEquals(
                (cell2minus1 - cell2 + cell1plus1 - cell1),
                shm.getDelta(id2, id1, 0, 0, 0),
                EPSILON);
    }
    ...
    public void getDelta_validRegions_calculatesValue() {
        ...
        assertEquals(
                (regionminus3 - region),
                shm.getDelta(id, Region.NUCLEUS.ordinal(), Region.DEFAULT.ordinal(), 0, 0, 0),
                EPSILON);
        assertEquals(
                (regionplus2 - region),
                shm.getDelta(id, Region.DEFAULT.ordinal(), Region.NUCLEUS.ordinal(), 0, 0, 0),
                EPSILON);
    }
    ...
    public void getSurface_validIDs_calculatesValue() {
        ...
        assertEquals(
                lambda * Math.pow(surface - targetSurface + 1, 2), shm.getSurface(id, 1), EPSILON);
        assertEquals(
                lambda * Math.pow(surface - targetSurface - 1, 2), shm.getSurface(id, -1), EPSILON);
    }
    ...
    public void getSurface_validRegions_calculatesValue() {
        ...
        assertEquals(
                lambda * Math.pow(surface - targetSurface, 2),
                shm.getSurface(id, region.ordinal(), 0),
                EPSILON);
        assertEquals(
                lambda * Math.pow(surface - targetSurface + 1, 2),
                shm.getSurface(id, region.ordinal(), 1),
                EPSILON);
        assertEquals(
                lambda * Math.pow(surface - targetSurface - 1, 2),
                shm.getSurface(id, region.ordinal(), -1),
