...
public class AdhesionHamiltonianTest {
    ...
    private static final double[] ADHESION_ID = {
        0, randomDoubleBetween(0, 10), randomDoubleBetween(0, 10)
    };
    ...
    static class AdhesionHamiltonianMock extends AdhesionHamiltonian {
        AdhesionHamiltonianMock(PottsSeries series, Potts potts) {
            super(series, potts);
        }

        @Override
        double getAdhesion(int id, int x, int y, int z) {
            return ADHESION_ID[id];
        }

        @Override
        double getAdhesion(int id, int region, int x, int y, int z) {
            return ADHESION_REGION[region];
        }
    }

    @Test
    public void constructor_called_initializesMaps() {
        AdhesionHamiltonianMock ahm =
                new AdhesionHamiltonianMock(mock(PottsSeries.class), mock(Potts.class));
    ...
    public void constructor_called_initializesParameters() {
        ...
        series.potts.put(
                "adhesion/ADHESION" + TARGET_SEPARATOR + key1 + TARGET_SEPARATOR + "*",
                adhesion1[0]);
        series.potts.put(
                "adhesion/ADHESION" + TARGET_SEPARATOR + key1 + TARGET_SEPARATOR + key1,
                adhesion1[1]);
        series.potts.put(
                "adhesion/ADHESION" + TARGET_SEPARATOR + key1 + TARGET_SEPARATOR + key2,
                adhesion1[2]);

        series.potts.put(
                "adhesion/ADHESION" + TARGET_SEPARATOR + key2 + TARGET_SEPARATOR + "*",
                adhesion2[0]);
        series.potts.put(
                "adhesion/ADHESION" + TARGET_SEPARATOR + key2 + TARGET_SEPARATOR + key1,
                adhesion2[1]);
        series.potts.put(
                "adhesion/ADHESION" + TARGET_SEPARATOR + key2 + TARGET_SEPARATOR + key2,
                adhesion2[2]);

        double[] adhesionDefault =
                new double[] {
                    randomDoubleBetween(1, 100), randomDoubleBetween(1, 100),
                };
        double[] adhesionNucleus =
                new double[] {
                    randomDoubleBetween(1, 100), randomDoubleBetween(1, 100),
                };
        ...
        assertEquals(
                adhesionDefault[0],
                ahm.popToAdhesionRegion.get(2).get(Region.DEFAULT).get(Region.DEFAULT),
                EPSILON);
        assertEquals(
                adhesionDefault[1],
                ahm.popToAdhesionRegion.get(2).get(Region.DEFAULT).get(Region.NUCLEUS),
                EPSILON);
        assertEquals(
                adhesionNucleus[0],
                ahm.popToAdhesionRegion.get(2).get(Region.NUCLEUS).get(Region.DEFAULT),
                EPSILON);
        assertEquals(
                adhesionNucleus[1],
                ahm.popToAdhesionRegion.get(2).get(Region.NUCLEUS).get(Region.NUCLEUS),
                EPSILON);
    }
    ...
    public void register_withRegions_addsConfig() {
        ...
        assertEquals(adhesion[1], config.getAdhesion(1), EPSILON);
        assertEquals(
                adhesionNucleus.get(Region.NUCLEUS),
                config.getAdhesion(Region.NUCLEUS, Region.NUCLEUS),
                EPSILON);
    }
    ...
    public void getDelta_validRegions_calculatesValue() {
        AdhesionHamiltonianMock ahm =
                new AdhesionHamiltonianMock(mock(PottsSeries.class), mock(Potts.class));
        assertEquals(
                ADHESION_REGION[1] - ADHESION_REGION[2], ahm.getDelta(1, 2, 1, 0, 0, 0), EPSILON);
        assertEquals(
                ADHESION_REGION[2] - ADHESION_REGION[1], ahm.getDelta(1, 1, 2, 0, 0, 0), EPSILON);
    }
}
